...
<aside id="sidebar" class="flex flex-col px-4 py-3 -translate-x-full transition-transform duration-300 ease-out shadow shadow-slate-300 absolute w-full md:w-96 h-screen bg-slate-50 z-[15000] left-0">
  ...
  <nav class="flex-grow space-y-1">
    <a href="index.php" class="flex items-center gap-3 px-3 py-2 transition-colors duration-200 rounded-lg cursor-pointer hover:bg-slate-200 <?= strpos($currentURL, "pengaturan-user") === false ? 'shadow bg-gradient-to-b from-blue-400 to-blue-500 shadow-blue-300 text-white' : "" ?>">
      <i class="<?= strpos($currentURL, "pengaturan-user") === false ? 'text-white' : 'text-slate-400' ?> fa-solid fa-house-user"></i>
      <span>Utama</span>
    </a>
    <a href="pengaturan-user.php" class="flex items-center gap-3 px-3 py-2 transition-colors duration-200 rounded-lg cursor-pointer hover:bg-slate-200 <?= strpos($currentURL, "laporan") !== false ? 'shadow bg-gradient-to-b from-blue-400 to-blue-500 shadow-blue-300 text-white' : "" ?>">
      <i class="<?= strpos($currentURL, "pengaturan-user") !== false ? 'text-white' : 'text-slate-400' ?> fa-solid fa-chart-line"></i>
